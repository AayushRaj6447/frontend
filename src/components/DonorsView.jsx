    ...
    <div className="space-y-6 animate-fade-in pb-12">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 glass-panel p-4 sm:p-6 rounded-2xl sm:rounded-3xl">
        <div>
          <h1 className="text-xl sm:text-2xl font-extrabold text-slate-100 flex items-center gap-2">
            <UserPlus className="w-5 h-5 sm:w-6 sm:h-6 text-blood-500" />
      ...
      {filteredDonors.length === 0 ? (
        <div className="glass-panel p-8 sm:p-12 rounded-3xl text-center text-slate-400">
          ...
          <p className="font-semibold text-sm sm:text-base">No donors match your search or filter.</p>
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    {donor.profilePhoto ? (
                      <img
                        ...
                        alt={donor.firstName}
                        className="w-11 h-11 sm:w-12 sm:h-12 rounded-2xl object-cover border border-blood-500/40 shrink-0"
                      />
                    ) : (
                      <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-2xl bg-blood-950 border border-blood-500/30 flex items-center justify-center font-bold text-blood-300 text-base shrink-0">
                        {donor.firstName?.[0] || 'D'}
                      </div>
                    )}
                    <div className="min-w-0">
                      <h3 className="font-bold text-slate-100 text-sm sm:text-base group-hover:text-blood-400 transition-colors truncate">
                        {donor.firstName} {donor.middleName ? donor.middleName + ' ' : ''}{donor.lastName}
                      </h3>
                      <div className="text-xs text-slate-400 flex items-center gap-1">
                        <MapPin className="w-3 h-3 text-slate-500 shrink-0" />
                        <span className="truncate">{donor.fullAddress || 'Address not provided'}</span>
                      </div>
                    </div>
                  </div>

                  <span className="px-2.5 py-1 rounded-xl bg-blood-950/80 border border-blood-500/40 font-black text-blood-400 text-xs sm:text-sm shadow-inner shrink-0">
                    {donor.bloodType}
                  </span>
                </div>
      ...
      {(isAddModalOpen || editingDonor) && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-dark-900/80 backdrop-blur-md animate-fade-in">
          <div className="relative w-full max-w-xl max-h-[90vh] glass-panel-glow rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-2xl border border-blood-500/30 overflow-hidden flex flex-col">
            ...
            <form onSubmit={editingDonor ? handleEditSubmit : handleAddSubmit} className="space-y-3 overflow-y-auto pr-1 flex-1">
              ...
              {!editingDonor && (
                <div>
                  ...
                  <div className="flex items-center gap-3">
                    {photoPreview ? (
                      <img src={photoPreview} alt="Preview" className="w-10 h-10 rounded-xl object-cover border border-blood-500 shrink-0" />
                    ) : (
                    ...
                    <label className="flex-1 cursor-pointer flex items-center justify-center gap-2 px-3 py-2 rounded-xl border border-dashed border-blood-500/40 hover:border-blood-500 bg-blood-950/20 text-slate-300 text-xs font-medium truncate">
